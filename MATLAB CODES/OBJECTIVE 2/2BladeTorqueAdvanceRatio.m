...
for sheetNum = 1:numSheets  
    ...
    CQ_interp = interp1(J, CQ_all(sheetNum, :), J_interp, 'spline');

    % Plot individual curve with unique color and line style
    plot(J_interp, CQ_interp, lineStyles{sheetNum}, 'Color', colors(sheetNum, :), 'LineWidth', 2);
end

% Finalize plot with enhanced formatting
grid on;
...
title('Torque Coefficient vs. Advance Ratio', 'FontSize', 18, 'FontName', 'Times New Roman');
